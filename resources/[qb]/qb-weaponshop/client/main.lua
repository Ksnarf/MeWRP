...
RegisterNetEvent('qb-shops:client:UpdateShop1', function(shop, itemData, amount)
    TriggerServerEvent('qb-shops:server:UpdateShopItems', shop, itemData, amount)
end)

RegisterNetEvent('qb-shops:client:SetShopItems1', function(shop, shopProducts)
    Config.Locations[shop]["products"] = shopProducts
end)

RegisterNetEvent('qb-shops:client:RestockShopItems1', function(shop, amount)
                ...
                if dist < 10 then
                    ...
                    if dist < 1 then
                        ...
                        if IsControlJustPressed(0, 38) then -- E
                            local ShopItems = {}
                            ShopItems.items = {}
                            QBCore.Functions.TriggerCallback('qb-shops:server:getLicenseStatus1', function(hasLicense, hasLicenseItem)
                                ShopItems.label = Config.Locations[shop]["label"]
                                if Config.Locations[shop].type == "weapon" then
                                    if hasLicense and hasLicenseItem then
                                        ShopItems.items = SetupItems(shop)
                                        QBCore.Functions.Notify("Våben sælger tjekker dit sygesikringskort", "success")
                                        Wait(500)
                                    else
                                        for i = 1, #products do
                                            if not products[i].requiredJob then
                                        ...
                                        QBCore.Functions.Notify("Viser dig våben!", "error")
                                        Wait(500)
                                        QBCore.Functions.Notify("Snak med politi afdelingen for, at få våben tilladelse", "error")
                                        Wait(1000)
                                    end
                                else
                                    ShopItems.items = SetupItems(shop)
...
CreateThread(function()
	for store, _ in pairs(Config.Locations) do
		if Config.Locations[store]["showblip"] then
			...
			if Config.Locations[store]["products"] == Config.Products["weapons"] then
